...
import member2 from "../../assets/gigi.png"
  ...
  return (
    <div className='lg:h-screen bg-white'>
      <div className='flex flex-col lg:flex-row lg:items-center lg:h-screen font-custom'>
        <p
          id='my-text'
          className='vertical-text text-3xl px-7 lg:px-10 2xl:px-24 py-10 font-light lg:py-0 lg:text-[78px] 2xl:text-[128px] font-custom text-black lg:leading-[73.45px]'
        >
          Our Team
        </p>
        <p className='lg:w-[530px] text-justify tracking-tighter px-7 lg:px-10 2xl:pr-24 text-[#050505] text-base leading-[21.33px] font-[250]'>
          ...
          {teamMembers.map(member => (
            <div key={member.id} className='lg:ml-3 2xl:ml-2 relative'>
              ...
              <div className='lg:absolute lg:bottom-0 left-0 right-0 bg-[#0E0E0E] text-white p-4'>
                ...
                <p className='text-base lg:text-[20px] font-light text-[#D9D9D9]'>{member.role}</p>
              </div>
            </div>
          ))}
        </div>
