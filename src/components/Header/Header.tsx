...
const LogoLink: React.FC<{ onHover: () => void; onLeave: () => void }> = ({ onHover, onLeave }) => {
  ...
  return (
    ...
    >
      <svg
        className='w-[35px] h-[35px] lg:w-[58px] lg:h-[52px]'
        viewBox="0 0 58 52"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
      ...
      {isHovering && (
        <div className="logo-tooltip font-custom">
          <span className='font-custom font-bold px-2'>V E G Z A</span>
        </div>
      )}
    </Link>
  );
...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header
        className={`lg:w-[100%] relative px-4 w-full lg:px-[50px] py-4 lg:py-6 bg-gradient-to-r from-[#0a0a0a34] to-[#10111100] flex text-[16px] font-custom z-50 ${
        ...
        >
          <nav className="flex flex-col justify-center items-center text-left h-screen uppercase ">
            ...
            <NavLink
              to="/vegza-viz"
              onClick={closeMenu}
              className="text-white w-fit px-8 border border-[#A8FF00] hover:bg-[#A8FF00] hover:text-black mt-[18px] py-[8px] text-[24px] "
            >
              Vegza Viz
            </NavLink>
          </nav>
